...
def get_predictions(model, scaler, x, y, edge_index, edge_weight, num_nodes):
    ...
    with torch.no_grad():
        ...
        batch_size, num_feature, _, _ = x.size()
        xxx = x.view(-1, num_feature)
        qqqq = model(xxx, edge_index, edge_weight, batch_size, num_feature)

        _y_pred = scaler.inverse_transform(qqqq.view(len(x), -1).cpu().numpy()).reshape(-1)
